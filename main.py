...
def get_ali_sign(params):
    # חישוב חתימה מדויק לפי דרישות אליאקספרס
...
def convert_ali_link(url):
    try:
        ...
        params["sign"] = get_ali_sign(params)
        response = requests.get(api_url, params=params, timeout=10).json()
        
        logger.info(f"API Result: {response}")
