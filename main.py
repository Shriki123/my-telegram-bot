...
# High-precision handler for messages
@user_client.on(events.NewMessage(chats=SOURCE_CHANNELS))
async def handler(event):
    try:
        msg_text = event.message.message or ""
        # Precise regex for s.click links seen in Mattress and iPhone posts
        urls = re.findall(r'(https?://[^\s]*aliexpress[^\s]*)', msg_text)
        
        if urls:
            print(f"Deal detected in channel: {event.chat_id}")
            final_text = msg_text
            for url in urls:
...
async def main():
    ...
    # This will now only send ONCE on successful start
    print("🚀 System started and monitoring.")
