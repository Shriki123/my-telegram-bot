...
async def handler(event):
    # This is our radar - it prints everything to the log
    print(f"📡 DEBUG: Message detected! From ID: {event.chat_id}")
    
    # Check for Ali links
    text = event.message.message or ""
    if "s.click.aliexpress.com" in text:
        print(f"✅ Ali link found! Attempting to forward...")
        try:
            await b_cli.send_message(DESTINATION_ID, f"New Post Detected:\n\n{text}")
...
async def main():
    try:
        ...
        print(f"🟢 Connected as {me.first_name}. Listening for messages...")
