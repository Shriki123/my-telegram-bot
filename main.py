...
def get_affiliate_link(url): try: endpoint = "" params = {"app_key": APP_KEY, "tracking_id": TRACKING_ID, "urls": url, "timestamp": str(int(time.time() * 1000))} query_string = "".join(f"{k}{v}" for k, v in sorted(params.items())) sign_source = APP_SECRET + query_string + APP_SECRET params["_aop_signature"] = hashlib.md5(sign_source.encode('utf-8')).hexdigest().upper() response = requests.get(endpoint + APP_KEY, params=params, timeout=10) data = response.json() res_key = "aliexpress_open_api_getPromotionLinks_response" if res_key in data: result = data[res_key].get("resp_result", {}).get("result", {}) links = result.get("promotion_links", {}).get("promotion_link", []) if links: return links[0]["promotion_link"] except Exception: pass return url
