...
# פרטי גישה מהלוגים שלך
API_ID = 33305115
API_HASH = "b3d96cbe0190406947efc8a0da83b81c"
BOT_TOKEN = "8414998973:AAGis-q2XbatL-Y3vL8OHABCfQ10MJi5EWU"

# ערוצים מעודכנים לפי הדיוק שלך
...
def get_affiliate(url):
    try:
        ...
        p = {
            ...
            "source_value": res.url, "timestamp": str(int(time.time() * 1000)),
...
async def handler(event): await process_msg(event.message)

async def main():
    keep_alive()
    await b_cli.start(bot_token=BOT_TOKEN)
    # כאן הבוט ינסה להשתמש בקובץ ה-session שהעלית
    if not await u_cli.is_user_authorized():
        logger.error("🛑 הבוט לא מורשה! עליך להעלות את קובץ ה-session מהמחשב שלך ל-GitHub.")
        return
    
    logger.info("🚀 הבוט מחובר! בודק הודעות אחרונות...")
